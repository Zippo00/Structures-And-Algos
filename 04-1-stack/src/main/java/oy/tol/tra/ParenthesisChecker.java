    ...
    */
    public static int checkParentheses(StackInterface<Character> stack, String fromString) throws ParenthesesException {
      ...
      for(int i = 0; i < fromString.length();i++){
         if(first.contains(""+fromString.charAt(i))){
            parenthesesAmount++;
            try{
               stack.push(fromString.charAt(i));
            }catch(StackIsEmptyException E){
               throw new ParenthesesException("Parentheses push failed", ParenthesesException.STACK_FAILURE);
            }
         }
         if(second.contains(""+fromString.charAt(i))){
            parenthesesAmount++;
            try{
               popped = stack.pop();
            }catch(StackIsEmptyException E){
               throw new ParenthesesException("Parentheses pop failed - too many closing parentheses", 
            ...
            if(fromString.charAt(i) == ')'){
               if (popped == '('){
                  ...
                  throw new ParenthesesException("Wrong kind of parenthesis in text", 
      ...
      if(stack.isEmpty() == false){
         throw new ParenthesesException("Text has more opening than closing parenthesis", ParenthesesException.TOO_FEW_CLOSING_PARENTHESES);
      }
      return parenthesesAmount;
   }
}
