...
public class InvoiceInspector {
   ...
   /**
    * Implement this method so that the time complexity of creating new
   ...
   public void handleInvoicesAndPaymentsFast() {
      ...
      dueDate.set(Calendar.MONTH, dueDate.get(Calendar.MONTH)+1);
      long dueDateValue = dueDate.getTime().getTime();
      Algorithms.fastSort(payments, 0, payments.length-1);
      Algorithms.fastSort(invoices, 0, invoices.length-1);
      for(int i = 0; i < invoices.length; i++){
         Payment toSearch = new Payment(invoices[i].number, 0);
         int paymentsIndex = Algorithms.binarySearch(toSearch, payments, 0, payments.length-1);
         if (paymentsIndex != -1){
            if(invoices[i].sum.compareTo(payments[paymentsIndex].sum) > 0){
               toCollect.add(new Invoice(invoices[i].number, invoices[i].sum - payments[paymentsIndex].sum, dueDateValue));
            }
         }else{
            toCollect.add(invoices[i]);
         }
      }
   }
}
